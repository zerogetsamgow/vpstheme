...
Key points: • Use markers to show your actual data points to distinguish the series from a continuous distribution.
However, if there are many categories or the values are approximate, use a line chart without markers.
• Use lines between markers to show trends.
Use dashed lines if you want the focus to be on the markers.
• *Do not* use a line chart if you are displaying categories on the X axis.
Use a bar chart instead.
• *Do not* use more than 2 colours in one chart.
If the data series is a ‘nice to have’, push them to the background by using a dark grey colour.



```{r  line_plus, fig.width = 7, fig.height = 5}
# example plot
last_plot() +
  guides(colour = "none") +
  geom_text(
...
ggplot(
  ...
  geom_segment(data = tibble(x = 2021.5, xend = 2021.5, y = 60, yend = 170, type = NA), 
               aes(x = x, y = y, xend = xend, yend=yend), colour = bv.charcoal, fill = NA)+
...
Ensure high contrast between values, by using `scale_*_dh()` as shown above or highlighting one value as shown below.

The examples below show how to achieve this using either `geom_text()` or `facet_grid()`.
