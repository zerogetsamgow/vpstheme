...
This package is a prototype. While it has useable palettes and themes for the Department of Health and Department of Families, Fairness and Housing as well as a range of helper functions, these palettes and themes require refinement.
...
The themes at present only different in the background colours that can be specified
...
Each theme is built on `theme_vps_foundation`. This theme is designed to be a foundation from which to build new themes, and not meant to be used directly. {theme_vps_foundation() is a complete theme with only minimal number of elements defined. It is easier to create new themes by extending this one rather than `theme_gray` or `theme_bw` from ***ggplot2***, because those themes define elements deep in the hierarchy. This theme takes {[ggthemes]{theme_foundation}()} and sets base text family and size to `Rubik` and `15`.
...
-   `make_tints` allows you to simply create Brand Victoria compliant tint colour palettes ranging from a specified start colour to white in five linear increments. White is then excluded from the palette to leave you with four tints of the original colour.
