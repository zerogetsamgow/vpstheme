...
Version `r packageVersion("vpstheme")` includes useable palettes and themes for the Department of Health, the Department of Families, Fairness and Housing and the Department of Justice and Community Safety as well as a range of helper functions, these palettes and themes require refinement.
...
ggplot2::ggplot(
  ...
  ggplot2::geom_point(shape = 22, size = 20, show.legend = FALSE) +
  ggplot2::geom_text(
    ggplot2::aes(x = group + .2), 
    hjust = 0,
    colour = bv.charcoal, 
    size = 4) +
  ...
  ggplot2::scale_y_continuous(
    limits = c(-6.6,-0.4), expand = c(0,0)) +
  ggplot2::scale_x_continuous(
    limits = c(.75,2.75),expand = c(0,0)) +
  ggthemes::theme_map()
```
...
ggplot2::ggplot(
  ...
  ggplot2::geom_point(shape = 22, size = 23, show.legend = FALSE) +
  ggplot2::geom_text(colour = vpstheme::bv.white, size = 3) +
  ggplot2::scale_fill_manual(
    values = vpstheme::bv_colours$complete
  ) +
  ggplot2::scale_y_continuous(limits = c(-3,-1), expand = c(.8,.8)) +
  ggplot2::scale_x_continuous(limits = c(1,10),expand = c(.05,.05)) +
  ggthemes::theme_map()
...
-   [Department of Justice and Community Safety](https://www.justics.vic.gov.au/) - `djcs_colours` is based on the departments standard templates, with a `primary` colour for each template and `tints` for each primary colour.

```{r djcs_tints, out.height="400px", out.width="700px"}
scales::show_col(unlist(djcs_colours[1]), ncol = 3)

```
```{r djcs_colours, out.height="400px", out.width="700px"}
...
-   [Department of Justice and COmmunity Safety](https://www.justice.vic.gov.au/) - `theme_vps_djcs()`.

```{r djcs_setup_djcs, include = FALSE}
...
```{r theme_vps_djcs, out.height="400px", out.width="700px"}
...
-   the base_colour parameter of `theme_vps_djcs` can take values `navy`, `charcoal`, and `purple` that correspond to the department's core colours.
