---
title: "Selecting and presenting graphs in R for DJCS."
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Selecting and presenting graphs in R for DJCS.}
...
This vignette was written for staff in the Department of Justice and Community Safety.
...
# Using theme_vps_djcs

A theme comprises information on the colours, fonts and line styles you want the base of your plot to use. ***vpstheme*** exports several themes including `theme_vps_djcs()`.


We can add `theme_vps_djcs()` to a plot to implement many of the requirements of the departmental guidelines as shown below.


```{r with_theme, fig.width = 7, fig.height = 5}
# Add theme_vps_djcs() to last_plot()
last_plot() +
  theme_vps_djcs()
```
...
We can add titles, subtitles and captions to ggplot objects using `labs()` as shown below. As we have applied `theme_vps_djcs()`, these additions comply with the departmental guidelines requirement that the title be placed in top left so it can be read before the y-axis label.
...
This is achieved in `theme_vps_djcs` by setting `hjust = .8` for axis.text.y via `element_text()`. This shifts the axis title justification to be closer to the top of the axis.
...
This adjustment is made by setting `x = NULL` in `labs()` or `name = NULL` in `scale_x_*()` rather than being a feature of `theme_vps_djcs.`
...
This is achieved in `theme_vps_djcs()` using the `lighten()`function from the ***colorspace*** package.
...
The Department of Justice and Community Safety's primary colour, named `bv.navy` in ***vpstheme*** can be used as a fill by adding it as follows.

```{r add_colour, fig.width = 7, fig.height = 5}

airport_plot +
  geom_col(fill = bv.navy)
...
We can highlight Milan in our airport example by adding `fill = city` to our aesthetics and assigning `"Milan" = bv.navy` in scale_fill_manual as shown below. Note, as we haven't assigned a colour to the other cities, they take their colour from na.value, which is `bv.charcoal`.
...
airport_plot +
  ...
  scale_fill_manual(values = c("Milan"=bv.navy), guide = "none")
...
***vpstheme*** includes `scale_fill_djcs` and `scale_fill_djcs` that enable you to easily apply shades of the department's primary palette to a chart (as well as more colourful discrete palettes to use where appropriate).


We revisit our accidents example, but add more detail to allow us to show how to use `scale_fill_djcs` to create charts with shades of a single colour. Without `scale_fill_djcs` our chart presents as follows.
...
`scale_fill_djcs` allows us to easily specify a palette comprised of departmental style guide compliant colours. We specify the `"purples"` palette in the example below.


``` {r add_scale_fill_djcs, warnings = FALSE, fig.height = 5, fig.width = 7}
last_plot() +
  # Add scale_fill_djcs and specify the purples palette
  scale_fill_djcs(palette = "purples", name = NULL) 
```

For this example, a legend is required as we use multiple colours. Departmental guidelines recommend legends be placed as close as possible to content, below the title, close to the graph data (for example, at the end of a line) but not cluttering the plot area. Legend and plot margins in `theme_vps_djcs()` have been set to achieve the recommended spacing, but can be adjusted using `theme()`. 


By default, `theme_vps_djcs()` places the legend below the graph as shown above, but this can be adjusted using the `theme()` function when you are using `position_stack()` to create stacked column charts. This is shown in the example below. `ggplot` assigns colours from left to right and top to bottom.  Using `position_stack()` would place "M1" at the top of each column and shade it `bv.navy`. You can simply reorder the colours by setting `reverse = TRUE` in  `scale_fill_djcs`, though this will  




``` {r stack_scale_fill_djcs, warnings = FALSE, fig.height = 5, fig.width = 7}
last_plot() +
 ...
 # scale_fill_djcs(palette = "purples", name = NULL, reverse = TRUE) +
...
For line charts, it is recommended that you highlight a single line of interest by colouring it and using shades of black for other lines. The function `scale_colour_djcs_line` has been created for this purpose. This scale creates a palette comprising a single colour and four shades of black. You can specify the colour (choosing from navy, blue, red and orange) and three different options for the shades of black (dark, light or gradient).

We create a new example to illustrate how to use `scale_colour_djcs_line` as shown below. This highlights "Region A" while other regions are shown in grey.
...
ggplot(
  ...
  scale_colour_djcs_line(name = NULL, colour = "cyan") +
  theme_vps_djcs() 
```
...
Other style guide recommendations include the use markers to show your actual data points to distinguish the series from a continuous distribution. We illustrate this and another option for `scale_colour_djcs_line` below.

``` {r multi_line, fig.width = 7, fig.height = 5}
# example plot
last_plot() +
  geom_point(show.legend = FALSE) +
  scale_colour_djcs_line(guide = "none", colour = "purple", gradient = TRUE) 
```
...
May not be required.
However, should you choose to include them to help users read and compare values if differences in data are hard to perceive, then this is accommodated in `theme_vps_djcs()`.
...
accident_plot +
  theme_vps_djcs(show_gridlines = TRUE)
```
...
ggplot2::ggplot(
  ...
  # na.value
  scale_fill_manual(guide = "none", values = c("M3" = bv.cerulean), na.value = bv.smoke)+
