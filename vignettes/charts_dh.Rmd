...
## Using theme_vps_dh to format non-data components
...
The following example illustrates how `theme_vps_dh()` implements the guidelines with discussion below.
...
ggplot(
  ...
  coord_flip()+
#  geom_text(aes(label = round(flights/1e6)), size = 5, hjust = 0, nudge_y  = .1e6)+
#  scale_fill_manual(guide = "none", values = c("Prague"=bv.pink), na.value = bv.smoke)+
...
Legend and plot margins in `theme_vps_dh()` have been set to achieve the recommended spacing, but can also be adjusted using `theme()`. YOu can also use `scale_*_continuvps()` a wrapper around `scale_*_continuous()` that has default expansion parameters to match departmental guidlines.

By default, `theme_vps_dh()` places the legend below the graph as shown below, but this can be adjusted using the `theme()` function when you are using `position_stack()`

```{r col_example_full_colour, warnings = FALSE, fig.height = 4, fig.width = 7}
...
ggplot(
  data = car_accidents,
  aes(x = month, y = accidents, fill = freeway, label = freeway)) +
  ...
  theme_vps_dh()
```
Ensure high contrast between values, by using `scale_*_dh()` as shown above or highlighting one value as shown below. List legend items in the same sequence as the bars they refer to, or matching the order of the end points on a line graph. We provide examples of how to achieve this below.

Legends are not required if only one series of values in the graph, as per the Airports example above, and may not be required if data components can be labelled. This is preferred. The examples below show how to achieve this using either `geom_text()` or `facet_grid()`.


```{r col_example_text_no_label, warnings = FALSE, fig.height = 4, fig.width = 7}
ggplot2::ggplot(
  data = car_accidents,
  ...
  theme_vps_dh()
```
```{r col_example_facet_no_label, warnings = FALSE, fig.height = 4, fig.width = 7}
ggplot2::ggplot(
  data = car_accidents,
...
The order of the bars depends on the categories they depict. • When there is a natural sequence we typically order the column left to right (for example, a series of dates may be ordered earliest to most recent). • If the dataset is not contiuous (eg. categorical or nominal comparisons), we advise to use horizontal bar charts instead. • Consider removing axis titles if they are self-explanatory, or fully explained in the heading.
